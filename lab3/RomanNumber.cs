...
public class RomanNumber : ICloneable, IComparable
{
    ...
    {
        char[] symbols = new char[] { 'M', 'D', 'C', 'L', 'X', 'V' };
        char[] symbols2 = new char[] { 'C', 'X', 'I' };
        string res = "";
        ...
        {
            int x = t / i;
            if (x == 9) res = res + symbols2[k] + symbols[k * 2];
            else if (x >= 5)
            {
                res += symbols[1 + k * 2];
                for (int j = 0; j < x - 5; ++j) res += symbols2[k];
            }
            else
            {
                if (x == 4) res = res + symbols2[k] + symbols[1 + k * 2];
                else
                {
                    for (int j = 0; j < x; ++j) res += symbols2[k];
                }
            }
            t = t % i;
        }
